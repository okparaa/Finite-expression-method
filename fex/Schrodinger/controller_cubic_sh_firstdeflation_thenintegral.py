...
logging.basicConfig(level=logging.INFO, filename='fexlog.log', format="%(asctime)s %(levelname)s %(message)s")
...
def get_reward(bs, actions, learnable_tree, tree_params, tree_optim):
    ...
    for bs_idx in range(batch_size):
        ...
        for _ in range(20):
            mse = torch.mean(torch.abs((learnable_tree(x_int, bs_action)) ** 3))
            function_error = torch.nn.functional.mse_loss(func.LHS_pde(learnable_tree(x, bs_action), x, dim),
                                                          func.RHS_pde(x))
            loss = function_error * (mse ** (-1))  # *(integral-integral_true)**2 #+ 100*integration + 100*bd_error
        ...
        function_error = torch.nn.functional.mse_loss(func.LHS_pde(learnable_tree(x, bs_action), x, dim),
                                                      func.RHS_pde(x))
        mse = torch.mean(torch.abs((learnable_tree(x, bs_action)) ** 3))
...
def best_error(best_action, learnable_tree):
    ...
    integral = torch.mean(learnable_tree(x_int, bs_action))
